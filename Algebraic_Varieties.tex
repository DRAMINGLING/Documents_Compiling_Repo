...
\end{proposition}
\begin{definition}
    Let $V, W$ be two affine varieties, $\phi: V \rightarrow W$ a regular map.
    We say $\phi$ is isomorphic if there exists regular map $\psi: W \rightarrow V$ such that $\psi \circ \phi = \identity_{V}$ and $\phi \circ \psi = \identity_{W}$.
\end{definition}
\begin{corollary}
    Let $V, W$ be two affine varieties, $\phi: V \rightarrow W$ a regular map.
    Then $\phi$ is isomorphic if and only if $\phi^{\ast}$ is isomorphic.
    \label{Corollary 1.3}
\end{corollary}
\begin{definition}
...
\vspace{-\baselineskip} % 减少一行间距
\begin{lemma}
    Let $\mathbb{P}_{k}^{n}$ be projective space over $k$ with coordinate $x_{0}, x_{1}, \cdots, x_{n}$.
    Then for all $0 \le i \le n$, the open subset $D_{+}(x_{i}) := \{x_{i} \neq 0\}$ is homeomorphic to affine space $\mathbb{A}_{k}^{n}$ with Zariski topology.
    \label{Lemma 2.2}
\end{lemma}
Hence, we can always embed affine variety into projective space.
In particular, we have that $\mathbb{P}_{k}^{0} \cong \mathbb{A}_{k}^{1}$.
...
\begin{remark}
    ...
    However, now we have not enough knowledge to give a proof.
\end{remark}
\begin{definition}
...
\begin{example}
    ...
    we can define a regular map $\mathbb{P}_{k}^{n} \longrightarrow \mathbb{P}_{k}^{m}$ defined by
    \begin{equation}
        [x_{0}, x_{1}, \cdots, x_{n}] \longmapsto [F_{0}(x_{0}, \cdots, x_{n}), F_{1}(x_{0}, \cdots, x_{n}), \cdots, F_{m}(x_{0}, \cdots, x_{n})]
    \end{equation}
...
\begin{definition}
    Let $X, Y$ be (quasi-)projective varieties.
    Consider pairs $(U, f_{U})$ where $U \subset X$ is an open dense subset and $f_{U}: U \rightarrow Y$ is a regular map.
    We $(U, f_{U}) \sim (V, f_{V})$ if $f_{U}\big{|}_{U \cap V} = f_{V}\big{|}_{U \cap V}$.
...
\end{proof}
\begin{remark}
    For irreducible variety $X$, taking affine open dense subset $U \subseteq X$,
    by (c), $K(X) = K(U) = \fraction(A(U))$ is computable.
\end{remark}
\begin{example}
...
\begin{remark}
    ...
    it induces a canonical map $f^{\ast}: K(Y) \rightarrow K(X)$ mapping $g$ to $g \circ f$, which is a homomorphism of $k$-algebras.
\end{remark}
\begin{proposition}
...
\begin{proposition}
    ...
    In particular, when $m = n$, the diagonal set $\Delta_{\mathbb{P}_{k}^{n}}$ defined by $\{x_{i}y_{j} - x_{j}y_{i}\}_{0 \le i,j \le n}$ is a Zariski closed subset.
...
\begin{remark}
    ...
    In particular, $(\mathbb{P}_{k}^{n} \times \mathbb{P}_{k}^{m}, \text{ product topology}) \rightarrow (\mathbb{P}_{k}^{mn + m + n}, \text{ Zariski topology})$ is a continuous map.
    Thus the image is also irreducible.
\end{remark}
Now embed general varieties into projective spaces, we can define product of varieties.
\begin{definition}
    Let $X, Y$ be varieties.
    Assume $X \subseteq \mathbb{P}_{k}^{n}$ and $Y \subseteq \mathbb{P}_{k}^{m}$.
    Define the product $X \times Y$ with subspace topology of $\mathbb{P}_{k}^{n} \times \mathbb{P}_{k}^{m}$.
\end{definition}
\begin{remark}
    In particular, when $X = \mathbb{A}_{k}^{n}$ and $Y = \mathbb{A}_{k}^{m}$,
    we have that $\mathbb{A}_{k}^{n} \times \mathbb{A}_{k}^{m} \cong \mathbb{A}_{k}^{m + n}$.
\end{remark}
\begin{proposition}
    Let $X, Y$ be varieties.
    Then \\
...
\end{reason}
\begin{remark}
    As any variety can be embedded into some projective space as a locally closed subset,
    we can also define graph for general regular map $X \rightarrow Y$.
\end{remark}
...
Consider the projection $\pi_{0}: \mathbb{P}_{k}^{n} \dashrightarrow \mathbb{P}_{k}^{n - 1}$ sending $[x_{0}, \cdots, x_{n}]$ to $[x_{1}, \cdots, x_{n}]$,
which is a regular map outside $0$.
\par
Set $U = \mathbb{P}_{k}^{n} \setminus \{0\}$.
Consider the closure of $\Gamma_{\pi_{0}}$ in $\mathbb{P}_{k}^{n} \times \mathbb{P}_{k}^{n - 1}$, called the blow up of $\mathbb{P}_{k}^{n}$ at $0$ and denoted by $Bl_{0}\mathbb{P}_{k}^{n}$.
\begin{lemma}
    Denote coordinate of $\mathbb{P}_{k}^{n}$ by $x_{0}, \cdots, x_{n}$ and coordinate of $\mathbb{P}_{k}^{n - 1}$ by $y_{1}, \cdots, y_{n}$.
    Then $Bl_{0}\mathbb{P}_{k}^{n} \subseteq \mathbb{P}_{k}^{n} \times \mathbb{P}_{k}^{n - 1}$ is defined by the bihomogenous polynomials $\{x_{i}y_{j} - x_{j}y_{i} \big{|} 0 \le i,j \le n\}$.
    \label{Lemma 3.2}
\end{lemma}
\begin{proof}
    Obviously, $Bl_{0}\mathbb{P}_{k}^{n} \subseteq V(\{x_{i}y_{j} - x_{j}y_{i}\})$.
    Want to show that there is a covering consisting of irreducible subsets $V_{i} \subseteq V(\{x_{i}y_{j} - x_{j}y_{i}\})$ such that $\Gamma_{\pi_{0}} \cap V_{i}$ contains some nonempty open subset of $V_{i}$.
    \par
    Consider projection maps
    \begin{equation}
        \begin{tikzcd} 
            & V(\{x_{i}y_{j} - x_{j}y_{i}\}) \subseteq \mathbb{P}_{k}^{n} \times \mathbb{P}_{k}^{n - 1} \arrow[dr, "p_{2}"] \arrow[dl, "p_{1}" swap] & \\
    ...
    It is clear that $p_{1} \circ p_{2}^{-1}(\beta)$ is the line connecting $0$ and $[0, b_{1}, \cdots, b_{n}]$.
    \par
    For each $U_{i} = D_{+}(y_{i}) \subseteq \mathbb{P}_{k}^{n - 1}$, 
    since $x_{i}y_{j} - x_{j}y_{i} = y_{i}(x_{i}\frac{y_{j}}{y_{i}} - x_{j})$ for all $j$, we have that
    \begin{equation}
        \begin{split}
            p_{2}^{-1}(U_{i}) & \cong p_{1} \circ p_{2}^{-1}(U_{i}) \\
            & = \{[x_{0}, x_{i}\frac{y_{1}}{y_{i}}, \cdots, x_{i}\frac{y_{n}}{y_{i}}]\} \\
            & \cong \{[x_{0}, x_{i}] \times (\frac{y_{1}}{y_{i}}, \cdots, \frac{y_{n}}{y_{i}})\} \\
            & = \mathbb{P}_{k}^{1} \times U_{i}
        \end{split}
    \end{equation}
    which is an open subset of $\mathbb{P}_{k}^{1} \times \mathbb{P}_{k}^{n - 1}$ hence irreducible. 
    Thus $p_{2}^{-1}(U_{i})$ is irreducible for all $i$.
    \par
    For each $\alpha = [a_{0}, \cdots, a_{n}] \neq 0$, we have that $p_{1}^{-1}(\alpha) = \alpha \times [a_{1}, \cdots, a_{n}]$.
    Hence $p_{1}^{-1}(W_{i}) \overset{\sim}{\rightarrow} W_{i}$, where $W_{i} = D_{+}(x_{i}) \subseteq \mathbb{P}_{k}^{n}$.
...
\begin{example}
    ...
    \begin{equation}
        \begin{tikzcd}
            Bl_{p_{1}, p_{2}, p_{3}}\mathbb{P}_{k}^{2} \arrow[dr, rightarrow, "\phi"] \arrow[d] & \\
...
\begin{theorem}[\textbf{\emph{Noether-Castelnuovo}}]
    Any birational map $\varphi\in Cr(\mathbb{P}_{k}^{2})$ is of the form $u^{s} \circ M_{1} \circ u \circ M_{2} \circ \cdots M_{n} \circ u^{t}$,
    where $M_{i}\in SL_{k}(3)$ and $s,t\in \{0, 1\}$.
    \label{Theorem 3.2}
\end{theorem}
...
\begin{proof}
    ...
    In this case, embed $X$ into some $\mathbb{P}_{k}^{m}$ and consider a commutative diagram
    \begin{equation}
        \begin{tikzcd}
    ...
    \par
    Step 3: For general variety $X$, want to show that $X \times \mathbb{P}_{k}^{n} \rightarrow X$ is closed.
    ...
    Since $Y$ is projective, we can embed it into $\mathbb{P}_{k}^{n}$ as a closed subset for some $n$.
    Consider the following commutative diagram
    \begin{equation}
        \begin{tikzcd}
            X \times Y \arrow[r, "\pi_{X}"] \arrow[d, hookrightarrow] &
            X \arrow[d, equal] \\
            X \times \mathbb{P}_{k}^{n} \arrow[r, "\pi"] &
            X
        \end{tikzcd}
    \end{equation}
    As $X \times Y$ is a closed subset of $X \times \mathbb{P}_{k}^{n}$ and $\pi$ is closed,
    it is clear that $\pi_{X}$ is also closed.
\end{proof}
\begin{corollary}
...
\end{corollary}
\begin{proof}
    Let $f: X \rightarrow \mathbb{A}_{k}^{1}$ be a regular function.
    Then $\widetilde{f}: X \rightarrow \mathbb{A}_{k}^{1} \hookrightarrow \mathbb{P}_{k}^{1}$ is a regular map.
    Hence $\widetilde{f}$ is closed and $\widetilde{f}(X)$ is closed in $\mathbb{P}_{k}^{1}$.
...
\begin{corollary}
    ...
    Assume that $X$ is connected and $X$ is not a one-point set.
    Then for all hypersurface $H \subseteq \mathbb{P}_{k}^{n}$, $X$ meets $H$.
    \label{Corollary 3.3}
\end{corollary}
\begin{proof}
    Say $H = V(F)$ where $F$ is a homogeneous polynomial of degree $d$.
    Consider $d$th Veronese map $V_{d}: \mathbb{P}_{k}^{n} \hookrightarrow \mathbb{P}_{k}^{\binom{n + d}{d} - 1}$.
    Denote $N = \binom{n + d}{d} - 1$.
    Suppose that $F = \sum_{I} a_{I}x^{I}$, where $I$ varies over $(a_{0}, a_{1}, \cdots, a_{n})$ satisfying that $\sum a_{i} = d$ and $x_{I} = \prod_{i} x_{i}^{a_{i}}$.
    ...
    Taking $G$ varies over $x^{I}$ for all $I$, it is clear that $v_{d}(X)$ is a point.
...
\end{remark}
\begin{proposition}
    Let $X$ be an irreducible affine variety, $A(X)$ coordinate ring.
    Then $\dim X = \dim A(X)$.
...
\begin{theorem}
    ...
    Then $\dim A = \trdeg(\fraction(A)/k)$ the Krull dimension of $A(X)$.
    \label{Theorem 4.1}
\end{theorem}
\begin{corollary}
...
\end{corollary}
\begin{reason}
    By taking affine open subset $U$ meeting $Y$, we can reduce to the case that $X$ is affine.
    Then apply preceding theorem to $A(X)$, we are done.
\end{reason}
\begin{proposition}
    Let $X$ be a variety.
...
\begin{example}
    ...
    (3)Assume that $X$ is irreducible, then $\dim X = \trdeg(K(X)/k)$ since we can take affine open dense subset $U \subseteq X$ and hence
    \begin{equation}
        \dim X = \dim U = \trdeg(K(U)/k) = \trdeg(K(X)/k)
    \end{equation}
    (4)Let $f: X \dashrightarrow Y$ be a dominant rational map between irreducible varieties.
    Then $f$ would correspond to a field extension $K(Y) \hookrightarrow K(X)$.
    Hence $\dim Y = \trdeg (K(Y)/k) \le \trdeg (K(X)/k) = \dim X$.
    \label{example:some_elementary_examples_in_dimension_theory}
\end{example}
Though $K(X)$ may be not a field when $X$ is not irreducible,
we can generalize the last entry in Example \ref{example:some_elementary_examples_in_dimension_theory}.
\begin{proposition}
    Let $f: X \dashrightarrow Y$ be a dominant rational map.
    Then for all irreducible component $W$ of $Y$, there is a dominant rational map $\widetilde{f}: \cup_{i} V_{i} \dashrightarrow W$,
    where $V_{i}$ are all irreducible components of $X$ meeting $f^{-1}(W)$.
    Moreover, we have that $\dim W \le \max_{i} \{\dim V_{i}\}$ and in particular, $\dim Y \le \dim X$.
    \label{Proposition 4.4}
\end{proposition}
\begin{proof}
    Assume $U$ is the domain of definition of $f$.
    Then $f^{-1}(W)$ would be union of some irreducible components of $U$, say $f^{-1}(W) = \cup_{i} (V_{i} \cap U)$.
    And we get a dominant rational map $\widetilde{f}: \cup_{i} V_{i} \dashrightarrow W$.
    \par
    Note that $W = \overline{\cup_{i} f(V_{i} \cap U)} = \cup_{i} \overline{f(V_{i} \cap U)}$.
    As $W$ is irreducible, $W = \overline{f(V_{i} \cap U)}$ for some $i$.
    Then by same argument as (4) in Example \ref{example:some_elementary_examples_in_dimension_theory},
    we get that $\dim W \le \dim V_{i} \le \max_{i} \{\dim V_{i}\}$. 
\end{proof}
...
\begin{theorem}
    ...
    Then we have that \\
    (1)If $X$ is of pure dimension $n$ and $X \cap V(F_{1}, \cdots, F_{r})$ is nonempty, 
    then each irreducible component of $X \cap V(F_{1}, \cdots, F_{r})$ is of dimension $\ge n - r$. \\
    (2)If $X$ is closed of dimension $n$ and $r \le n$, then $X \cap V(F_{1}, \cdots, F_{r}) \neq \varnothing$.
    \label{Theorem 4.4} 
\end{theorem}
\begin{remark}
...
\begin{reason}
    ...
    Then $V(F) = \cup_{i} V(F_{i})$ and by Krull's Principal Theorem and Proposition \ref{Proposition 4.1}, 
...
\end{definition}
\begin{definition}
    Let $f: X \rightarrow Y$ be a morphism of varieties, $\sharp k$ uncountable.
    Assume $\mathcal{P}$ is a property of fibers.
...
\end{definition}
\begin{theorem}
    Let $X, Y$ be irreducible affine varieties, $f: X \rightarrow Y$ a dominant morphism.
...
\begin{proof}
    ...
    \par
    Induct on $n$. 
    For $n = 1$, denote coordinate of $\mathbb{A}_{k}^{1 + m}$ by $x, y_{1}, \cdots, y_{m}$.
    Then $A(X)$ is generated by $\overline{x}$ over $A(Y)$.
    If $x$ is algebraic over $K(Y)$, assume $G(T)\in K(Y)[T]$ is the minimal polynomial of $x$.
    ...
    \par
    For all $y\in Y$ and $(\alpha, y)\in f^{-1}(y)$, as soon as $b_{0}(y) \neq 0$,
    ...
    written as $b_{0}(y_{1}, \cdots, y_{m})x^{d} + b_{1}(y_{1}, \cdots, y_{m})x^{d - 1} + \cdots + b_{d}(y_{1}, \cdots, y_{m})$,
    we have that $b_{j}(y) = 0$ for all $y\in Y$ so that $b_{j}(y_{1}, \cdots, y_{m})\in I(Y)$ for all $j$.
...
\begin{remark}
    ...
    In addition, $\dim X = \dim Y$ and for general $y$, $\sharp f^{-1}(y) \le [K(X) : K(Y)]$ taking equality when $\characteristic k = 0$.
\end{remark}
\begin{example}
...
\begin{lemma}
    ...
    Assume that $U$ is an affine open dense subset of $Y$ and $V \subseteq f^{-1}(U)$ is an affine open dense subset of $X$.
    Then a general fiber of $f$ is finite if and only if a general fiber of $f\big{|}_{V}$ is finite.
    \label{Lemma 4.1}
\end{lemma}
\begin{proof}
    The \leftquotes $\Rightarrow$ \rightquotes arrow is trivial since for all $y\in U$, $f\big{|}_{V}^{-1}(y) \subseteq f^{-1}(y)$.
    For the converse, since a general fiber of $f\big{|}_{V}$ is finite, by Theorem \ref{Theorem 4.5}, $\dim X = \dim V = \dim U = \dim Y$.
...
\end{corollary}
\begin{proof}
    Take affine open dense subset $U \subseteq Y$ and affine open dense subset $V \subseteq f^{-1}(U)$.
...
\end{corollary}
\begin{proof}
    We may assume that $X \subseteq \mathbb{P}_{k}^{N}$ is a closed subset for some $N \ge n$.
    When $N = n$, the only closed subset of $\mathbb{P}_{k}^{n}$ of dimension $n$ is itself hence we can take the identity map.
    ...
    Hence by Krull's Principal Ideal Theorem, $V(f) \cap L_{p}$ is of codimension $1$ in $L_{p}$ so that $X \cap L_{p} \subseteq V(f) \cap L_{p}$ is a finite set.
    \par
    Let $Y = \overline{\pi(X)} \subseteq \mathbb{P}_{k}^{N - 1}$.
    By Theorem \ref{Theorem 4.5}, $\dim Y = \dim X = n$.
    Hence by induction, there exists a dominant morphism $Y \rightarrow \mathbb{P}_{k}^{n}$ with all fibers finite.
...
\begin{proof}
    ...
    Then there exist $f: X \rightarrow \mathbb{P}_{k}^{\dim X}$ dominant with all fibers finite and $g: X \rightarrow \mathbb{P}_{k}^{\dim Y}$ dominant with all fibers finite.
    Consider $f \times g: X \times Y \rightarrow \mathbb{P}_{k}^{\dim X} \times \mathbb{P}_{k}{\dim Y}$,
    which is dominant with all fibers finite.
    Note that $\dim \mathbb{P}_{k}^{\dim X} \times \mathbb{P}_{k}{\dim Y} = \dim \mathbb{A}_{k}^{\dim X} \times \mathbb{A}_{k}{\dim Y} = \dim \mathbb{A}_{k}^{\dim X + \dim Y} = \dim X + \dim Y$.
...
\begin{proof}
    ...
    Note that $C^{0}(X) \cap C^{0}(Y) = (C^{0}(X) \times C^{0}(Y)) \cap \Delta_{\mathbb{A}_{k}^{n + 1}}$ and by Corollary \ref{Corollary 4.7}, $\dim (C^{0}(X) \times C^{0}(Y)) = \dim X + \dim Y + 2$,
...
\begin{example}
    ...
    Then $p^{-1}(\mathbb{P}_{k}^{n} \setminus \{x\}) \overset{p}{\rightarrow} \mathbb{P}_{k}^{n} \setminus \{x\}$ is an isomorphism and $p^{-1}(x) \cong \mathbb{P}^{n - 1}$.
\end{example}
\begin{definition}
...
\begin{proof}
    ...
    Hence $Y_{i} \cap H_{y}$ is of dimension $d - 1$ for all $Y_{i}$ containing $y$.
    Then take $Y' = \cup_{Y_{i}\ni y} (Y_{i} \cap H)$ which is of pure dimension $d - 1$.
    By induction, done!
\end{proof}
\begin{remark}
    In fact, with argument above, we get a stronger result that we can take $F_{1}, \cdots, F_{d}$ to be linear equations.
\end{remark}
\begin{theorem}
...
\begin{proof}
    ...
    there is some irreducible component $X'$ of $X_{x}$ containing $x$ have dimension $\ge \dim X - \dim Y + 1$.
    \par
    Thus $X' \subseteq X(\dim X - \dim Y + 1)$ and $X' \subseteq Z$ as $X'$ irreducible.
    If we take $U = X \setminus (\cup Z')$, where $Z'$ varies over all irreducible components of $X(\dim X - \dim Y + 1)$ except $Z$,
...
\begin{example}
    ...
    \end{equation}
    Since $\varphi^{-1}(\alpha)$ has dimension $\ge 1$, the above system of equations cannot only have finitely many solutions.
...
\begin{proof}
    ...
    \par
    By Proposition \ref{Proposition 4.5}, each $\{y\in Y \big{|} d_{i}(y) \ge r\}$ is closed.
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{split}
    \end{equation}
    while for all $\alpha\in q(\Gamma_{\varphi}) \subseteq Y \times Z$, $q^{-1}(\alpha) = X \times \{\alpha\}$.
    Since $q^{-1}(\alpha)$ also has dimension $\dim X$,
    we get that $q^{-1}(\alpha) = q\big{|}_{\Gamma_{\varphi}}^{-1}(\alpha)$.
    In particular, for all $y$ and $z = \varphi(0, y)$, take $\alpha = (y, z)$ and we get $q^{-1}(\alpha) = X \times \{y\} \times \{z\}$.
    Conclude that $X \times \{y\}$ is contracted to $\{z\}$.
\end{proof}
\begin{definition}
    Let $X$ be a variety.
    Assume that $X$ admits a group structure $(X, \cdot, \iota, e)$,
    where multiplication $\cdot: X \times X \rightarrow X$, inverse $\iota: X \rightarrow X$ and identity $e$ satisfy group structure.
...
\begin{proof}
    ...
    By Corollary \ref{Corollary 4.9}, there exists nonempty open subset $U \subseteq Y$ such that for all $y\in Y$,
...
\end{example}
\begin{proof}
    Since $X$ is projective, we can embed it into some projective space $\mathbb{P}_{k}^{N}$ as a closed subset.
    Consider the following commutative diagram
    \begin{equation}
        \begin{tikzcd}
            \Gamma_{\varphi} \arrow[r, hookrightarrow] \arrow[d, "\sim"] & 
            Y \times X \arrow[r, hookrightarrow] &
            Y \times \mathbb{P}_{k}^{N} \arrow[d, "p"] \\
            X \arrow[rr, "\varphi"] && 
            Y
        \end{tikzcd}
    \end{equation}
    Replace $X$ by graph which is a closed subvariety of $Y \times \mathbb{P}_{k}^{N}$.
    ...
    Hence $\varphi^{\ast}(a_{i}) = ab_{i - 1} - b_{i}$, where $b_{m} = 0$.
    \par
    For $n > m$, let $f(T) = \sum_{i = 0}^{m} \varphi^{\ast}(a_{i})T^{n - i}$.
...
Easy to $D_{\theta}$ satisfies Leibniz's Law and hence is a derivation.
\begin{definition}
...
\begin{reason}
    ...
    For all $x\in X$, $T_{x}X = \{(a_{1}, \cdots, a_{n}) \big{|} \sum_{i} a_{i}\frac{\partial F}{\partial x_{i}}(x) = 0, \forall F\in I(X)\}$.
    ...
    \begin{equation}
        \begin{split}
            ...
            \iff & \text{ for all $(N - r + 1)$-minor, determinant vanishes}
        \end{split}
    \end{equation}
    \vspace{-\baselineskip} % 减少一行间距
\end{reason}
...
\begin{proof}
    ...
    For all $p\in X$, $T_{p}X$ is defined by a linear polynomial $\sum_{i} \frac{\partial F}{\partial x_{i}}(p)x_{i}$.
    ...
    \par
    Suppose that $X_{n}^{c}$ is nonempty.
    Then since $X$ is irreducible, $X_{n}^{c} \cap U$ is also irreducible, contradicting to ${X'_{n}}^{c} = \varnothing$.
...
\begin{lemma}
    Let $X$ be a variety.
    Assume $V$ and $U$ are two closed subvarieties of $X$.
...
\end{proof}
\begin{remark}
    If $X$ is smooth at $x$, then $x$ cannot be contained in two distinct irreducible components since $\mathcal{O}_{X, x}$ is a regular local ring,
...
\begin{example}
    Let $X = V(x_{1}x_{2} - x_{3}x_{4}) \subseteq \mathbb{A}_{k}^{4}$.
    Then $\mathcal{O}_{X, 0}$ is not UFD.
    Consider $\varphi: \mathbb{A}_{k}^{3} \rightarrow X$ sending $(y_{1}, y_{2}, y_{3})$ to $(y_{1}, y_{2}y_{3}, y_{2}, y_{1}y_{3})$.
    Note the field extension $\varphi^{\ast}: K(X) \rightarrow K(y_{1}, y_{2}, y_{3})$ is an isomorphism and hence $\varphi$ is birational morphism.
...
\begin{proof}
    ...
    Take a basis $a_{1}, \cdots, a_{n - r}$ of $\overline{\mathfrak{m}_{x}}/\overline{\mathfrak{m}_{x}}^{2} \cong \mathfrak{m}_{x}/(\mathfrak{m}_{x}^{2} + \mathfrak{p})$.
...
\begin{proof}
    ...
    \par
    Note that for each irreducible component $Z_{0}$ of $\varphi^{-1}(Z)$ with $z\in Z_{0}$ mapping to $y$,
...
\begin{proof}
    ...
    By induction, we consider the case that $n = 1$ with projection $\mathbb{A}_{k}^{m + 1} \rightarrow \mathbb{A}_{k}^{m}$ sending $(x_{1}, \cdots, x_{m + 1})$ to $(x_{1}, \cdots, x_{m})$.
    \par
    ...
    By Proposition \ref{Proposition 5.2}, 
    there is always an open dense subset $V$ of $Y$ consisting of smooth points in $Y$.
    \par
    ...
    Take $U = D(\frac{\partial G}{\partial x_{m + 1}}) \cap \varphi^{-1}(V)$.
...
\begin{proof}
    ...
    Hence by Proposition \ref{Proposition 4.6}, $I_{X}$ is irreducible and $\dim I_{X} = \dim X + n - 1$.
    ...
    \par
    Now apply generic smoothness to $I_{X} \overset{p_{2}}{\rightarrow} {\mathbb{P}_{k}^{n}}^{\vee}$,
...
\begin{proof}
    ...
    \par 
    When $X \subseteq H$, the defining equation $F$ of $H$ should be contained in $I(X)$.
    ...
    Hence by Proposition \ref{Proposition 4.6}, $S_{X}$ is irreducible and $\dim S_{X} = n - 1$.
...
\end{proof}
\begin{theorem}[\textbf{\emph{Bertini's Theorem}}]
    Let $X$ be an irreducible variety, $\varphi: X \rightarrow \mathbb{P}_{k}^{n}$ a morphism and $\dim \overline{\varphi(X)} \ge 2$.
...
\begin{proof}
    ...
    \par
    Hence by induction, applying Proposition \ref{Proposition 4.6}, we get that $I$ is irreducible of dimension $\dim \grassmannianscheme{m - 1}{n - 1} + (n - 1) = (n - m)(m - 1) + (n - 1)$.
    Note that $p_{1}$ is surjective, $\grassmannianscheme{m}{n} = p_{1}(I)$ is also irreducible.
    Again by Proposition \ref{Proposition 4.6}, $\dim \grassmannianscheme{m}{n} + (m - 1) = (n - m)(m - 1) + (n - 1)$.
    Thus $\dim \grassmannianscheme{m}{n} = (n - m)m$.
    \par
    Consider group action $\generallineargroup{m}{k} \curvearrowright \grassmannianscheme{m}{n}$.
...
\begin{proof}
    ...
    Conclude that $\mathcal{P}(\grassmannianscheme{m}{n}) \setminus \ker \varphi \cong \mathbb{A}_{k}^{m(n - m)}$ which gives a birational map $\grassmannianscheme{m}{n} \dashrightarrow \mathbb{P}_{k}^{m(n - m)}$.
\end{proof}
\begin{example}
...
\end{example}
To introduce more interesting properties about line bundles,
...
\end{proposition}
\begin{proof}
    Obviously, for all homogeneous polynomial $f$ of degree $d$, as $\frac{f}{x_{i}^{d}} = \frac{x_{j}^{d}}{x_{i}^{d}} \cdot \frac{f}{x_{j}^{d}}$,
    we get $\{\frac{f}{x_{i}^{d}}\}$ is a section of $\mathcal{O}_{\mathbb{P}_{k}^{n}}(d)$.
...
\end{remark}
\begin{proposition}
    Let $L = (\{U_{r}\}, g_{rl})$ be a line bundle over irreducible variety $X$.
    Assume $s_{0}, \cdots, s_{n}\in \Gamma(X, L)$ are not all zero sections, where $s_{i} = \{s_{ir}\}$.
    Define a rational map $\phi_{L}: X \dashrightarrow \mathbb{P}_{k}^{n}$ by gluing up $U_{r} \dashrightarrow \mathbb{P}_{k}^{n}$ sending $x$ to $[s_{0r}(x), \cdots, s_{nr}(x)]$.
    If for all $x\in X$, $s_{ir}(x) \neq 0$ for some $i, r$,
    then $\phi_{L}$ is a well defined morphism.
    And in this case, $L = \phi_{L}^{\ast}\mathcal{O}_{\mathbb{P}_{k}^{1}}(1)$.
    \label{Proposition 7.2}
\end{proposition}
\begin{proof}
    Clearly, $\phi_{L}$ is a morphism, want to show that $(\{\phi_{L}^{-1}(D_{+}(x_{i}))\}, \frac{x_{j}}{x_{i}} \circ \phi_{L}) \sim (\{U_{r}\}, g_{rl})$.
    In fact, for all $x\in U_{r} \cap \phi_{L}^{-1}(D_{+}(x_{i}))$, by definition, $s_{ir}(x) \neq 0$.
    Hence $s_{ir}$ is an invertible regular function on $U_{r} \cap \phi_{L}^{-1}(D_{+}(x_{i}))$.
    And $g_{rl}s_{jl} = s_{jr} = \frac{s_{jr}}{s_{ir}} \cdot s_{ir} = (\frac{x_{j}}{x_{i}} \circ \phi_{L}) \cdot s_{ir}$, done!
\end{proof}
\begin{remark}
    For reducible $X$, this construction of rational map also makes sense if $U_{r} \cap (\cup_{i} D(s_{ir}))$ is dense in $U_{r}$ for all $r$.
\end{remark}
\begin{example}
    ...
    In fact, $v_{d}$ is given by sections in $\Gamma(\mathbb{P}_{k}^{n}, \mathcal{O}_{\mathbb{P}_{k}^{n}}(d))$ that are monomial of degree $d$ with coefficient $1$.
    \par
...
All these Weil divisors form an abelian subgroup of $Weil(X)$, called the group of principal Weil divisors on $X$.
\begin{example}
...
\begin{corollary}
    Let $X$ be an irreducible variety.
    Then for all rational function $f\in K(X)^{\ast}$ such that $\divisor (f) = nV$ with $n > 0$,
    we have that $f$ is a regular function and $V(f) = V$.
    In particular, when $n = 1$, locally the ideal of $A(X)$ corresponding $V$ is generated by $f$.
    \label{Corollary 7.1} 
\end{corollary}
\begin{reason}
    Similar to proof of Lemma \ref{Lemma 7.4}, we know $f$ is a nonzero regular function and $V$ is the only irreducible subvariety of $X$ of codimension $1$, which is contained in $V(f)$.
    By Krull's Principal Theorem, we know that $V(f)$ is of pure codimension $1$ so that $V(f) = V$.
    \par
    When $n = 1$, take any affine open subset $U$ meeting $V$, 
    if $g\in I(V \cap U) \subseteq A(U)$, then $V \cap U \subseteq \divisor (g)$ so that $\divisor (g) - V \cap U$ is an effective divisor on $V \cap U$.
    Note that $\divisor (f)\big{|}_{U}$ is just $V \cap U$, we get $\divisor (\frac{g}{f}) = \divisor (g) - V \cap U$ is effective.
    Hence $\frac{g}{f}$ is regular.
    Conclude that $I(V \cap U) = (f)$.
\end{reason}
...
\begin{remark}
    ...
    \begin{itemize}
        \item when $X$ is smooth, $\pi$ is isomorphic.
...
\begin{example}
    ...
    As $\mathcal{O}_{\mathbb{P}_{k}^{n}}(m)$ are nontrivial for all $m \neq 0$, conclude that $\picardscheme (X) = Cl(X) \cong \mathbb{Z}$. \\
...
\end{proof}
\begin{proposition}
    Let $X$ be a smooth projective irreducible curve not isomorphic to projective line $\mathbb{P}_{k}^{1}$.
...
\end{proposition}
\begin{proof}
    The \leftquotes $\Leftarrow$ \rightquotes arrow have been discussed in Example \ref{example:picard_group}.
