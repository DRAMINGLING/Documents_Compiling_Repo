...
\subsection{Spectrum}
\label{subsection:Schemes_Spectrum}
...
\subsection{Sheaves}
\label{subsection:Schemes_Sheaves}
...
\label{subsection:Schemes_Varieties}
...
\begin{definition}
    Let $k$ be an algebraically closed field, $X$ abstract prevariety over $k$.
    $X$ is an abstract variety if $\delta = \{(x, x) \big{|} x\in X\}$ is closed in $X \times X$. 
\end{definition}

\subsection{Structure sheaf}
\label{subsection:Schemes_Structure_sheaf}
...
\begin{reason}
    ...
    The harder part is to show this map is surjective, which need some commutative algebra to prove.
    ...
    Consider $\Delta_{ij} = a_{i}f_{j}^{n_{j}} - a_{j}f_{i}^{n_{i}}$ and annihilator $\annihilator_{A}(\Delta_{ij})$.
    If $f_{i}f_{j}\notin rad(\annihilator_{A}(\Delta_{ij}))$, take maximal ideal $\mathfrak{m}_{ij} \supseteq \annihilator_{A}(\Delta_{ij})$.
    Then $\mathfrak{m}_{ij}\in D(f_{i}f_{j})$ and so $\frac{a_{i}}{f_{i}^{n_{i}}} = \frac{a_{j}}{f_{j}^{n_{j}}}$ in $A_{\mathfrak{m}_{ij}}$.
    Thus there exists $s_{ij}\in (A \setminus \mathfrak{m}_{ij})$ such that $s\Delta_(ij) = 0$ and so $s\in \annihilator_{A}(\Delta_{ij})$.
    But $\annihilator_{A}(\Delta_{ij}) \subseteq \mathfrak{m}_{ij}$, get $s\in \mathfrak{m}_{ij}$, contradiction!
    \par
    Thus $f_{i}f_{j}\in rad(\annihilator_{A}(\Delta_{ij}))$ and there exists $k_{ij}\in \mathbb{M}$ such that $(f_{i}f_{j})^{k_{ij}}\Delta_{ij} = 0$.
...
\end{reason}
\begin{corollary}
    Let $A$ be a ring, $\mathcal{O}_{\spec A}$ the structure sheaf.
    Then $\mathcal{O}_{\spec A}(\spec A) = A$.
    \label{Corollary 1.2}
\end{corollary}
\begin{remark}
    With the structure sheaf, now we can distinguish $\spec k$ and $\spec k'$ for different fields $k$ and $k'$.
\end{remark}
\begin{proposition}
    Let $A$ be a ring, $\mathcal{O}_{\spec A}$ the structure sheaf.
...
\subsection{Schemes}
\label{subsection:Schemes_Schemes}
...
\label{subsection:Schemes_Projective_schemes}
...
\label{section:Morphisms_of_Schemes}
...
\label{subsection:Morphisms_of_Schemes_S-schemes}
...
\begin{proof}
    ...
    By Zariski's Lemma that any field extension of finite type as algebras is finite extension,
    $B/\mathfrak{m}$ is finite over $k(y)$ so that it is algebraic over $k(y)$, inducing $tr\dim(k(x)/k) = tr\dim(k(y)/k)$.
\end{proof}
...
\label{subsection:Sheaves_of_Modules_Modules_over_ringed_space}
...
\label{subsection:Sheaves_of_Modules_Quasi-coherent_and_coherent_sheaves}
...
\label{subsection:Sheaves_of_Modules_Direct_and_inverse_images}
...
\begin{proof}
    Can assume $X = \spec(B)$ and $Y = \spec A$.
    Then $\mathcal{G} = \widetilde{M}$ for some finitely generated $A$-module M so that $f^{\ast}\mathcal{G} = \widetilde{M \otimes_{A} B}$.
...
\label{subsection:Sheaves_of_Modules_Quasi-coherent_sheaves_on_projective_schemes}
...
\label{subsection:Sheaves_of_Modules_Ample_and_very_ample_invertible_sheaves}
...
\begin{proof}
    ...
    \begin{equation*}
        \begin{tikzcd}
            \mathfrak{b} \arrow[r, hookrightarrow] \arrow[d, "\varphi"] &
            A \arrow[dl, "\psi"] \\
            J &
        \end{tikzcd}
    \end{equation*}
...
\begin{proof}
    ...
    For $\mathcal{F} = \mathcal{O}_{X}(m)$, this is just what Theorem \ref{Theorem 7.6} said.
...
\begin{reason}
    ...
    Apply Theorem \ref{Theorem 7.7} to each $X \times_{S} U_{j} \rightarrow U_{j}$ and restriction of $\mathcal{F}$,
...
\end{proof}
\begin{remark}
    In fact, all finiteness results in Theorem \ref{Theorem 7.7} still hold for $X$ proper over $A$.
...
\begin{proof}
    ...
    \begin{equation*}
        H^{i}(X, \mathcal{F}) \otimes_{A} A' \longrightarrow H^{i}(X', v^{\ast}\mathcal{F})
    \end{equation*}
...
\label{subsection:Divisors_Riemann-Roch_for_curves}
...
\end{proof}
\begin{corollary}
    Let $(X, \mathcal{O}_{X})$ be a ringed space, $\mathcal{L}$ locally free of finite rank, $\mathcal{G}$ an $\mathcal{O}_{X}$-module.
...
\label{section:Cohomology_and_Base_Change}
...
\end{equation*}
If $f: X \longrightarrow Y$ affine, then $u^{\ast}f_{\ast}\mathcal{F} \longrightarrow g_{\ast}v^{\ast}\mathcal{F}$ is isomorphic.
...
Want to construct a scheme parameterizing all closed subschemes of $\mathbb{P}_{k}^{n}$.
...
Recall that a locally closed immersion $X \longrightarrow Y$ is a composition
\begin{equation*}
...
\begin{proof}
    ...
    Since finiteness is equivalent to finitely generated and integral, we get $B_{f}$ is finite over $A_{f}[f_{1}, \cdots, f_{n}]$.
...
\begin{proof}[\textbf{\emph{Construction of Hilbert Scheme}}]
    ...
    By Construction, $\hilbertscheme_{X/S}^{P} \hookrightarrow \grassmannianscheme{Q(m_{Q})}{F}$ is locally closed immersion.
