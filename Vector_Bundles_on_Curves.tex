...
\begin{definition}
    ...
    We say $\mathcal{L}$ is ample relative to $f$ (or say $f$-ample) if there exists affine open covering $U_{i}$ of $Y$ such that $\mathcal{L}\big{|}_{f^{-1}(U_{i})}$ is ample on $f^{-1}\big{|}_{f^{-1}(U_{i})}$.
\end{definition}
\begin{remark}
