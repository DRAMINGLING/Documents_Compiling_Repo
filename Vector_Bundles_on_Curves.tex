...
\begin{proof}
    ...
    By definition, $\divisor (s) = (U_{i}, f_{i})$ and $\mathcal{O}_{X}(\divisor (s))$ is defined by transition functions $\frac{f_{i}}{f_{j}} = g_{ij}$.
    Hence $\mathcal{O}_{X}(\divisor (s)) \cong \mathcal{L}$.
\end{proof}
\begin{corollary}
    Let $\mathcal{L}$ be an invertible sheaf on $X$.
...
\begin{reason}
    ...
    then we know $\mathcal{L} \cong \mathcal{O}_{X}(\divisor (s))$.
    Note that $s$ has no pole, $\divisor (s)$ is an effective Weil divisor so that $\deg \mathcal{L} = \deg \mathcal{O}_{X}(\divisor (s)) = \deg (\divisor (s)) \ge 0$.
\end{reason}
...
\begin{proof}
    ...
    \begin{equation}
        0 \rightarrow H^{0}(X, \mathcal{F} \otimes E^{j + 1}) \rightarrow H^{0}(X, \mathcal{F} \otimes E^{j}) \rightarrow H^{0}(X, \mathcal{F} \otimes S^{j}\mathcal{E}' \otimes S^{r - j}\mathcal{E}'') \rightarrow H^{1}(X, \mathcal{F} \otimes E^{j + 1}) \rightarrow \cdots
    \end{equation}
...
\label{subsection:Ample_Vector_Bundles_Vector_bundles_on_curves}
...
\begin{proposition}
    ...
    \label{Proposition 4.15}
\end{proposition}
\begin{proof}
    We first prove for invertible sheaf $\mathcal{M}$.
    Then by Lemma \ref{Lemma 4.4}, we can take $n_{1} = 2g - \deg M$.
...
\begin{proof}
    ...
    Then by Proposition \ref{Proposition 4.15}, we are done.
\end{proof}
\end{document}
