...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{tikzcd}
    \end{equation}
    By diagram chasing, it is easy to show that the medium column also exact.
...
\begin{proof}
    ...
    Since $\mathcal{L} \cong \mathcal{O}_{X}(D)$ for some divisor $D$,
    we can write $\mathcal{L} = \mathcal{O}_{X}(D_{1}) \otimes \mathcal{O}_{X}(-D_{2})$ with $D_{1}$ and $D_{2}$ effective.
    In fact, as $X$ is projective, for all different points $x$ and $y$ in $X$, $x - y$ is a principle divisor.
    Hence we can pick $D_{1}$ and $D_{2}$ disjoint.
    In fact, by same argument as above, we know $\overline{\mathcal{E}} = \sum_{i = 0}^{r - 1} \overline{\mathcal{L}_{i}}$ in $K_{0}(X)$.
\end{proof}
\end{document}