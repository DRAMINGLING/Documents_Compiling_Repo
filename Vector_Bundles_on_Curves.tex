...
\end{proof}
\begin{remark}
    It seems that the original statement in \emph{Ample Vector Bundles}, Hartshorne does not assume $X$ to be projective.
    And his proof for that case also used our short exact sequence above which requires $p^{\ast}\mathcal{F} \otimes \mathcal{L}^{n}$ to be flat.
    But for a general noetherian $X$, a coherent sheaf need not to be a quotient of a locally free sheaf.
    And globally generated property is not local so we cannot reduce to affine case.
    I donot know in such case, why we can assume $\mathcal{F}$ locally free.
    Maybe the derived push forward plays a role.
\end{remark}
...
\end{proposition}
\begin{proof}
    \leftquotes $\Rightarrow$ \rightquotes : 
    \par
