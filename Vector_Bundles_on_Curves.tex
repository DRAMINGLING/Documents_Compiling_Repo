...
\end{proof}
\begin{proposition}
    Let $f: X \rightarrow Y$ be a finite morphism between complete schemes, $\mathcal{L}$ ample invertible sheaf on $Y$.
    Then $f^{\ast}\mathcal{L}$ is ample invertible sheaf on $X$.
    \label{Proposition 4.5}
\end{proposition}
\begin{proof}
    In fact, here we only need affine morphism.
    In this case, the natural map $f^{\ast}f_{\ast}\mathcal{F} \twoheadrightarrow \mathcal{F}$ is surjective for all coherent sheaf $\mathcal{F}$,
    which can be affine locally defined as $M \otimes_{A} B \twoheadrightarrow M$.
    In addition, since $X$ and $Y$ are proper, by Cancellation Theorem, $f$ is proper hence $f_{\ast}\mathcal{F}$ is a coherent sheaf on $Y$
    \par
    ...
    Take inverse image, we get $\mathcal{O}_{X}^{\oplus I} \twoheadrightarrow f^{\ast}f_{\ast}(\mathcal{F} \otimes f^{\ast}\mathcal{L}^{n}) \twoheadrightarrow \mathcal{F} \otimes f^{\ast}\mathcal{L}^{n}$ for $n$ large enough.
...
\begin{proof}
    ...
    \end{equation}
    Then by Corollary \ref{Corollary 4.3}, we know $\mathcal{E}_{1}^{\oplus r}$ is ample.
...
\begin{proof}
    ...
    By Corollary \ref{Corollary 4.4}, $\mathcal{E} \otimes S^{n}\mathcal{E}$ is ample for $n$ large enough.
...
\end{corollary}
\begin{reason}
    Immediately comes from Proposition \ref{Proposition 4.10} and Corollary \ref{Corollary 4.3}.
\end{reason}
...
\end{corollary}
\begin{proof}
    First by Corollary \ref{Corollary 4.4}, we know $\mathcal{E}' \oplus \mathcal{E}'$ is ample.
...
\begin{proof}
    ...
    \par
    When $f: Y \rightarrow X$ is closed, for all $\mathcal{F}\in \coherent (Y)$,
    since $f$ is proper, $f_{\ast}\mathcal{F}$ is coherent on $X$.
    Then $f_{\ast}\mathcal{F} \otimes S^{n}\mathcal{E}$ is globally generated for $n$ large enough.
    Note that by Projection Formula, $f_{\ast}\mathcal{F} \otimes S^{n}\mathcal{E} \cong f_{\ast}(\mathcal{F} \otimes S^{n}\mathcal{E}\big{|}_{Y})$.
    As $f$ is affine, we have a natural surjection
    \begin{equation}
        f^{\ast}f_{\ast}(\mathcal{F} \otimes S^{n}\mathcal{E}\big{|}_{Y}) \twoheadrightarrow \mathcal{F} \otimes S^{n}\mathcal{E}\big{|}_{Y}
    \end{equation}
    While as $f_{\ast}(\mathcal{F} \otimes S^{n}\mathcal{E}\big{|}_{Y})$ is globally generated, we get $f^{\ast}f_{\ast}(\mathcal{F} \otimes S^{n}\mathcal{E}\big{|}_{Y})$ is globally generated and so is $\mathcal{F} \otimes S^{n}\mathcal{E}\big{|}_{Y}$.
    Conclude that $\mathcal{E}\big{|}_{Y}$ is ample.
\end{proof}
\end{document}
