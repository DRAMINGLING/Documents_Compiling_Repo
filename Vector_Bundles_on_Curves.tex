...
\end{corollary}
\begin{proof}
    Since $\mathcal{O}_{X} \overset{s}{\rightarrow} \mathcal{E}$ factors through $[s]$, we get a morphism $s: \mathcal{O}_{C} \rightarrow [s]$ so that $s$ is a nonzero global section of $[s]$ also.
    Hence $[s]$ is isomorphic to $\mathcal{O}_{C}(\divisor (s))$.
\end{proof}
\begin{remark}
