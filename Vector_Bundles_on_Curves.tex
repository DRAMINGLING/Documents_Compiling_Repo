...
\begin{theorem}[\textbf{\emph{Serre's Duality}}]
    ...
    Then for all locally free sheaf $\mathcal{F}(X)$ on $X$, we have canonical isomorphism $H^{i}(X, \mathcal{F}^{\vee} \otimes \omega_{X}) \overset{\sim}{\rightarrow} H^{n - i}(X, \mathcal{F})^{\vee}$.
    \label{Theorem 3.1}
\end{theorem}
\begin{remark}
